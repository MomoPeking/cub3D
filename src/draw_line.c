...
/*   Updated: 2020/09/30 17:46:12 by qdang            ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static void	draw_line_x(t_info *s, t_point p1, t_point p2, int color)
{
	...
	if (p2.x > p1.x)
		while (++i <= p2.x - p1.x)
			s->img_add[lround(p1.y + i * k) * s->sl + p1.x + i] = color;
	else
		while (++i <= p1.x - p2.x)
			s->img_add[lround(p2.y + i * k) * s->sl + p2.x + i] = color;
}
...
static void	draw_line_y(t_info *s, t_point p1, t_point p2, int color)
{
	...
	if (p2.y > p1.y)
		while (++i <= p2.y - p1.y)
			s->img_add[(p1.y + i) * s->sl + lround(p1.x + i * k)] = color;
	else
		while (++i <= p1.y - p2.y)
			s->img_add[(p2.y + i) * s->sl + lround(p2.x + i * k)] = color;
}
...
void		draw_line(t_info *s, t_point p1, t_point p2, int color)
{
	...
	if (abs_x == 0)
		while (++i <= abs_y)
			s->img_add[(ft_min(p2.y, p1.y) + i) * s->sl + p2.x] = color;
	else if (abs_y == 0)
		while (++i <= abs_x)
			s->img_add[p2.y * s->sl + ft_min(p2.x, p1.x) + i] = color;
	else
		abs_y <= abs_x ?
